...
// ── Firebase helpers ─────────────────────────────────────────────────────────
const STATE_DOC = "boyz-party/state";

async function saveToCloud(data) {
  try {
    await setDoc(doc(db, "boyz-party", "state"), data);
  } catch (e) {
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "boyz-party", "state"), (snap) => {
      ...
      setLoading(false);
      showToast("⚠️ Check Firebase config!");
    });
    return () => unsub();
  }, []);
  ...
  const toggleR = async (i, v) => {
    const newResults  = [...results]; newResults[i] = results[i] === v ? null : v;
    const newPayments = { ...payments }; delete newPayments[pk(i)];
    setResults(newResults); setPayments(newPayments);
    await persist({ members, results: newResults, payments: newPayments, sessions });
  };
  ...
  const toggleP = async (i) => {
    ...
    newPayments[k] ? delete newPayments[k] : newPayments[k] = true;
    setPayments(newPayments);
    await persist({ members, results, payments: newPayments, sessions });
  };

  const updateMemberName = async (i, name) => {
    const newMembers = [...members]; newMembers[i] = name;
    setMembers(newMembers);
    await persist({ members: newMembers, results, payments, sessions });
  };
  ...
  const saveSession = async () => {
    ...
    const newSessions = [entry, ...sessions.filter(s => s.date !== date)]
      .sort((a, b) => b.date.localeCompare(a.date));
    setSessions(newSessions);
    await persist({ members, results, payments, sessions: newSessions });
    showToast("Session saved ✓");
  };

  const resetDay = async () => {
    const newResults  = Array(9).fill(null);
    const newPayments = { ...payments };
    members.forEach((_, i) => delete newPayments[pk(i)]);
    setResults(newResults); setPayments(newPayments);
    await persist({ members, results: newResults, payments: newPayments, sessions });
    showToast("Reset! ✓");
  };
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
      ...
      <div style={{ color: "rgba(255,200,120,0.4)", fontSize: 13 }}>Syncing with cloud</div>
    </div>
  );
      ...
      <div style={{ ...card, marginBottom: 12 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ color: "#fff", fontSize: 20, fontWeight: 900 }}>
            ₹{totalColl} <span style={{ color: "rgba(255,200,120,0.35)", fontSize: 13, fontWeight: 400 }}>/ ₹{totalExp}</span>
          </div>
        </div>
      ...
      {members.map((name, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            {a && <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 7px", borderRadius: 6, background: r === "W" ? "rgba(74,222,128,0.12)" : "rgba(248,113,113,0.12)", color: r === "W" ? "#4ade80" : "#f87171", border: `1px solid ${r === "W" ? "rgba(74,222,128,0.2)" : "rgba(248,113,113,0.2)"}` }}>₹{a}</span>}
  ...
  const SummaryView = () => (
    <div style={{ paddingBottom: 20 }}>
      <div style={bigCard}>
        ...
        <div style={{ color: "rgba(255,200,120,0.35)", fontSize: 11, textAlign: "center", marginBottom: 16 }}>
          {paidList.length} paid · {pendingList.length} pending · {notSet.length} not set · {progress}%
        </div>
  ...
  const AllDaysView = () => {
    ...
    if (sessions.length === 0) return (
      <div style={{ textAlign: "center", padding: "60px 20px" }}>
        ...
        <div style={{ color: "rgba(255,200,120,0.25)", fontSize: 13, marginTop: 8 }}>Tracker → enter results → 💾 Save Session</div>
      </div>
    );
      ...
      <div style={{ paddingBottom: 20 }}>
        <div style={bigCard}>
          <div style={{ textAlign: "center", marginBottom: 14 }}>
            ...
            <div style={{ color: "rgba(255,200,120,0.35)", fontSize: 12 }}>{sessions.length} sessions · synced live ☁️</div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
            {[
              { label: "Collected", value: `₹${grandTotal}`, color: "#ffd700" },
              { label: "Expected",  value: `₹${grandExp}`,   color: "#ff9a3c" },
          ...
          <div style={{ color: "rgba(255,200,120,0.35)", fontSize: 11, textAlign: "center", marginTop: 6 }}>
            {grandExp > 0 ? Math.round(grandTotal / grandExp * 100) : 0}% collected overall
          </div>
        </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, fontFamily: "'Segoe UI',sans-serif", paddingBottom: 40 }}>
      ...
      <div style={{ display: "flex", margin: "0 16px 14px", background: "rgba(255,255,255,0.05)", borderRadius: 14, padding: 4, gap: 3 }}>
        {tabs.map(([t, label]) => (
          <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: "9px 4px", borderRadius: 10, border: "none", cursor: "pointer", background: tab === t ? "rgba(255,154,60,0.25)" : "transparent", color: tab === t ? "#ff9a3c" : "rgba(255,200,120,0.35)", fontSize: 12, fontWeight: tab === t ? 700 : 400, boxShadow: tab === t ? "0 0 16px rgba(255,154,60,0.2)" : "none", transition: "all 0.2s" }}>{label}</button>
        ))}
      </div>
